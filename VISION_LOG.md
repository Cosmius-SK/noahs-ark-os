...
## 12 Mar 2026 06:59 — Note
Token Ledger Redesign — per-scene, per-turn, per-character segregation. Current T108 sentinel accumulates tokens across the entire kernel session with no scene or turn boundaries. Needed: (1) Per-scene ledger — resets on New Scene, captures total prompt/completion/cost for that scene only. (2) Per-turn log — each turn records speaker, prompt tokens, completion tokens, cost, timestamp. (3) Per-character aggregation — total tokens consumed by each soul across a scene, and across all scenes. (4) Scene summary block appended to archive — token breakdown visible in the .txt file alongside Maya reflection. Long-term goal: feed this data into a metrics layer that identifies expensive characters, overlong prompts, context window bloat, and cost-per-narrative-quality ratio. This is the foundation for simulation optimisation — we cannot improve what we cannot measure at the right granularity. Tile impact: T108 TOKEN_SENTINEL redesign, T109 LOG_ARCHIVER receives structured token block, T111 scene-start resets per-scene counter. New data structure: token_log = {scene_id, turns: [{turn, speaker, prompt, completion, cost}], character_totals: {name: {prompt, completion, cost}}, scene_total: {prompt, completion, cost}}.

## 12 Mar 2026 07:17 — Note
Scene History Manager — scene-level manipulation interface. Global history is a flat list with no scene boundaries. Needed: (1) T109 writes a scene boundary marker into global_history on every SCENE_END — {type: SCENE_BOUNDARY, scene_id, timestamp, turn_count, environment_summary}. (2) T111 History panel tab shows a list of completed scenes derived from boundary markers — scene ID, turn count, timestamp, one-line env summary. (3) Noah can select and delete individual scene blocks from the list before a new run — the corresponding history entries are removed from global_history.json and the live state object simultaneously. (4) A Reload History button forces T107 to re-read global_history.json into memory, closing the file-vs-memory gap that currently causes stale context bleeds between runs. Without boundary markers this is impossible to build cleanly — T109 boundary writes should be added in Sprint 4 as a foundation step even before the UI panel exists. Tile impact: T109 writes markers, T107 gets reload_history(), T111 gets History tab panel. Sprint 4 foundation, Sprint 5 full UI.
